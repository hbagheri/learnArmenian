...
- [✅] **8b. Lesson runner UI** — 2026-06-06: Created `:feature:lessons` module with `LessonsListScreen` + `LessonRunnerScreen`. `LessonsListScreen` displays linear list of 5 lessons with progress bars and "شروع کنید" buttons; renders on device ✅. `LessonRunnerScreen` scaffolded to sequence steps by type. Navigation routes wired (`LessonsRoute`, `LessonRunnerRoute`). HomeScreen gets "ادامه‌ی درس" primary button. APK builds ✅. **Navigation debugged and verified end-to-end on device**: Home → (tap "ادامه‌ی درس") → LessonsListScreen → (tap lesson card) → LessonRunnerScreen ✅. Used `LessonsSharedViewModel` to pass lesson ID between screens. Step type rendering (letter/phrase/quiz/practice) still stub (shows hardcoded type name, not actual cards yet) — needs implementation.
- [ ] 7. SRS + streak
