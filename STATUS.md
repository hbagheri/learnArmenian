...
- [🟡] **8b. Lesson runner UI** — 2026-06-06: Created `:feature:lessons` module with `LessonsListScreen` + `LessonRunnerScreen`. `LessonsListScreen` displays linear list of 5 lessons with progress bars and "شروع کنید" buttons; renders on device ✅. `LessonRunnerScreen` scaffolded to sequence steps by type. Navigation routes wired (`LessonsRoute`, `LessonRunnerRoute`). HomeScreen gets "ادامه‌ی درس" primary button. APK builds ✅ and `LessonsListScreen` displays ✅, but navigation from list → runner needs debug (likely Compose navigation argument passing issue). Step type rendering (letter/phrase/quiz/practice) is stub (shows text, not actual cards yet).
- [ ] 7. SRS + streak
